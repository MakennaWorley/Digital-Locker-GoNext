...
import { addFile, buildDownloadUrl, deleteFile, fetchFiles, generateKey, type AccessKey, type VaultFile } from '@/lib/api';
import { AlertCircle, Check, Copy, ExternalLink, HardDrive, Key, Lock, Plus, RefreshCw, Shield, Trash2, X } from 'lucide-react';
		...
		>
			<div className="w-full max-w-md rounded-2xl border border-slate-700 bg-slate-800 shadow-2xl">
				...
				<div className="flex items-center justify-between border-b border-slate-700/60 px-6 py-4">
					...
					<button onClick={onClose} className="rounded-lg p-1.5 text-slate-400 transition-colors hover:bg-slate-700 hover:text-slate-200">
						<X className="h-4 w-4" />
					</button>
				</div>
				...
				<form onSubmit={handleSubmit} className="space-y-4 px-6 py-5">
					{/* Name input */}
					<div>
						<label className="block text-xs font-medium uppercase tracking-wider text-slate-500 mb-2">Asset Name</label>
						<input
							type="text"
							value={name}
					...
					{/* Path input */}
					<div>
						<label className="block text-xs font-medium uppercase tracking-wider text-slate-500 mb-2">Local Path</label>
						<input
							type="text"
							value={localPath}
				...
				<form onSubmit={handleSubmit} className="space-y-4 px-6 py-5">
					...
					<div>
						...
						<input
							...
							disabled={isLoading}
						/>
						<p className="mt-1 text-xs text-slate-600">Set to 1 for one-time download, 0 for unlimited</p>
					</div>

					{/* Hours valid */}
					<div>
						<label className="block text-xs font-medium uppercase tracking-wider text-slate-500 mb-2">Valid For (hours)</label>
						<select
							value={hoursValid}
			...
			<div className="w-full max-w-md rounded-2xl border border-slate-700 bg-slate-800 shadow-2xl">
				...
				<div className="space-y-4 px-6 py-5">
					...
					<div className="rounded-lg bg-slate-900/60 px-4 py-3">
						<p className="mb-1 text-xs font-medium uppercase tracking-wider text-slate-500">Asset</p>
						<p className="font-medium text-slate-200">{link.file.name}</p>
					</div>

					{/* Download URL */}
					<div className="space-y-2">
						<p className="text-xs font-medium uppercase tracking-wider text-slate-500">Download URL</p>
						<div className="flex items-center gap-2 overflow-hidden rounded-lg bg-slate-900/60 p-3 ring-1 ring-slate-700">
							<p className="min-w-0 flex-1 truncate font-mono text-sm text-emerald-400">{link.url}</p>
							<button
								onClick={handleCopy}
								title="Copy URL"
					...
					<div className="grid grid-cols-3 gap-3">
						<div className="rounded-lg bg-slate-900/60 px-4 py-3">
							<p className="mb-1 text-xs font-medium uppercase tracking-wider text-slate-500">Downloads</p>
						...
						<div className="rounded-lg bg-slate-900/60 px-4 py-3">
							<p className="mb-1 text-xs font-medium uppercase tracking-wider text-slate-500">Limit</p>
							<p className="text-slate-200">{link.key.max_uses > 0 ? `${link.key.max_uses}x` : '∞'}</p>
						</div>
						<div className="rounded-lg bg-slate-900/60 px-4 py-3">
							<p className="mb-1 text-xs font-medium uppercase tracking-wider text-slate-500">Expires</p>
							<p className="text-slate-200">
		...
		<div className="min-h-screen bg-slate-950 text-slate-100">
			...
			<header className="sticky top-0 z-40 border-b border-slate-800 bg-slate-900/70 backdrop-blur-md">
				<div className="mx-auto flex max-w-6xl items-center gap-3 px-6 py-4">
					...
					<div>
						<h1 className="text-lg font-bold leading-tight tracking-tight text-slate-100">Digital Locker</h1>
			...
			<div className="border-b border-slate-800/60 bg-slate-900/30">
				<div className="mx-auto max-w-6xl px-6 py-3">
					<div className="flex items-center gap-2 text-sm text-slate-400">
						...
						<span>
							<span className="font-semibold text-slate-200">{files.length}</span> secured {files.length === 1 ? 'asset' : 'assets'}
						</span>
					</div>
				</div>
			</div>
				...
				{loading && (
					<div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
						{Array.from({ length: 6 }).map((_, i) => (
							<div key={i} className="h-[9.5rem] animate-pulse rounded-xl border border-slate-700/30 bg-slate-800/40" />
						))}
					</div>
				)}
			...
			{/* ── Add File Modal ── */}
			<AddFileModal isOpen={showAddModal} onClose={() => setShowAddModal(false)} onSubmit={handleAddFile} isLoading={addingFile} />
